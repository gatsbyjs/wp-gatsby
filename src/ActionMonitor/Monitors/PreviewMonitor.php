...
class PreviewMonitor extends Monitor {
	...
	public static function get_gatsby_content_sync_url_for_post( $post ) {
		...
		$manifest_id = Preview::get_preview_manifest_id_for_post( $post );
	...
	public static function print_initial_preview_template_state_js() {
		...
		$initial_state = json_encode(
			[
				...
				'webhookWasCalled'       => Preview::was_preview_webhook_called_for_post_id(
					$post_id
				),
	 ...
	 * PHP context is related to Gatsby Preview
	 */
	public static function is_gatsby_preview() {
		$is_gatsby_preview 
			= \is_preview()
			|| isset( $_GET['preview_nonce'] )
			|| isset( $_GET['gatsby_preview'] );

		return $is_gatsby_preview;
	}
	...
	public function setup_preview_template( $template ) {
		...
		} elseif ( $is_gatsby_preview && ! $preview_url ) {
			return trailingslashit( dirname( __FILE__ ) ) . '../../Admin/includes/no-preview-url-set.php';
		}
